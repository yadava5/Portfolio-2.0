// HEADER ROUND 9 — the round-8 gates, re-run on the refined D.
//
// Same four measurements as probe-header7.mjs (LCP · idle-rAF census ·
// A7 pixel diff · errors), with waits sized for round 9's longer
// performance (~7.9s vs 2.9s). The numbers to hold, from round 8:
// rAF 0 · diff 0px · errors 0; LCP was 672 / 1246 / 60ms.
//
// Run:  node docs/design-lab/probe-header9.mjs
...
const OUT = ROOT + "design-lab/shots-header9";
...
async function measure(name, browserType, ctxOpts) {
  const browser = await browserType.launch();
  const url = `http://localhost:${PORT}/header-d-ensemble`;
  ...
  await page.waitForTimeout(8800); // the round-9 performance settles by ~8s
  ...
  const settledShot = `${OUT}/d9-${name}-settled.png`;
  ...
  const staticShot = `${OUT}/d9-${name}-static.png`;
  ...
  log({ candidate: "d9", seat: name, lcp, raf3s, diffPx, diffPct: pct, errors });
}

try {
  for (const [name, bt, opts] of SEATS) await measure(name, bt, opts);
} finally {
  server.kill();
  writeFileSync(`${OUT}/report9.json`, JSON.stringify(report, null, 2));
  console.log("report → " + OUT + "/report9.json");
}
