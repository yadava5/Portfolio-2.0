...
> started. **Phase 5 is next** — figures and benchmark artifacts. Read §1's
> C41 and C46 before it: Phase 4 retired the contrast gate and left the run's
> own palette ungated, which is Phase 5's to close.
...
| `check-contrast` | **Retire.** 589 lines mirroring a palette from `globals.css` and scanning `src/**/*.tsx`, which will not exist. `verify-portfolio` already prints "REACT ONLY" beside it. **The stronger reason, measured at the deletion: three of its twenty-one colours still ship. See C41 — and §5.2½, which is the replacement it leaves owing.** |
...
### 5.2½ — Give the run's palette a gate again. **Added by Phase 4.**

`check-contrast.mjs` is retired (C41) and nothing replaced it, so the run's own
ink, clay, pine and seven-waypoint day arc are measured by nobody. That gate's
defect was its SOURCE, not its maths: 589 lines of correct APCA and WCAG
composition over a palette hand-copied from a file that no longer exists, of
which three colours in twenty-one still shipped.

The replacement reads `src/run/index.html`'s own `:root` block — the run
declares `--ink`, `--ink-2`, `--clay`, `--clay-g`, `--pine`, `--thread`,
`--scrim-c` and their night companions in one place, twice (day and night), and
those are the colours a reader actually sees. Take the composition maths out of
the deleted file's history rather than rewriting it; what must not be carried
over is a table nothing binds. **The archive's own surfaces need it too** —
§3.7 measured them by hand in Phase 3 and no gate has read them since.
...
1. **The seam, both directions, from every station with a handoff** — the arrival
   ...
   station, not ¶04. The defect this was written about is fixed: `back to the
   work ⟵` → `/#work` (¶04, 08:47) on every file became
   `⟵ rejoin the line at ¶ NN · <station> — HH:MM`, and `check-crosswalk`'s
   fourth direction holds each one to `stations.ts` verbatim. What the walk is
   for now is whether it READS right — a gate can prove the hour is the correct
   one and cannot tell you whether arriving at it feels like coming back.
...
9. **Read the archive as a reader for the first time.** The 4.1 flip shipped the
   generated case files; until then they had only ever been read by gates and
   screenshots. This walk is the first time the rebuilt pages are read as pages.
10. **Wire or retire the nameplate's "machines withheld" guard (C46).**
    `nameplateMachines.ts` promises that a renamed author gets the plate without
    the machines, imports `personalInfo` for it, and never checks. `MACHINE`,
    the character-index map, is dead beside it. Both are ESLint warnings today;
    fixing either way is nameplate-engine work with a real browser gate behind
    it (`check-nameplate`, 28 invariants). **It also closes C44 for free:** the
    unused import is why `out/run/lib/data/personal.js` and `basePath.js` — 10.4
    KB no browser ever fetches — are still published, and removing or using the
    binding is the only thing that stops that.
11. **Rule on `docs/design-lab/`.** Phase 4 added it to ESLint's ignore list
    ...
    committed telemetry (already deferred in §6) plus the probes that produced
    it. Worth one ruling now that the tree around it is small.
