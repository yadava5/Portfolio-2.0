...
- **A9 — The living world (owner decision, 2026-07-30):** the owner reviewed three hero prototypes and chose a direction the NO-LIST as written forbids. The narrowings are recorded in **`docs/NO-LIST.md` §F** and are the only sanctioned relaxations: **F1** a warm, grain-bearing, arc-driven ink field is not the banned aurora (five binding conditions); **F2** bounded WebGL is permitted when it draws *only while the arc moves* and idles at zero frames — §E1's 🔴 was on *persistent* backgrounds. **F3 (one rAF loop) and F4 (one pin) are RE-AFFIRMED unchanged** — "a whole slide slides down" is satisfied by the composed-group entrance PREMIUM-FLOW §A already adopted, never by a second pin or by snap. **F5** dark remains the arc's night end, never a static default. A9 does not touch A8's recruiter guard: world-building must not add scroll length.
