...
import { LedgerGlance } from "@/components/paper/ProofGlance";
import { VisibilityGlyph } from "@/components/paper/proofGlyphs";
  ...
  return (
    <article data-dossier className="dossier-surface text-ink min-h-screen">
      ...
      <div className="relative mx-auto w-full max-w-[1240px] pt-28 pr-6 pb-16 pl-9 sm:px-12 xl:pr-16 xl:pl-36">
        ...
        {/* The glance strip (evviz round): the ledger's two-second read —
            one visibility mark per entry in ledger order (solid = public,
            half = private-safe, open+dash = local-only), the clay dash at
            a HELD entry's shoulder, and a tally caption computed from the
            same manifest rows the marks are drawn from — never typed. */}
        <LedgerGlance
          entries={proofManifest.map((entry) => ({
            ...
            return (
              ...
              >
                {/* evviz record head — the scan line: entry number on the
                    left, the visibility grade (drawn glyph + the exact
                ...
                <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1">
                  ...
                  <p className="label-mono text-ink-secondary">
                    ...
                    <span
                      aria-hidden="true"
                      className="mr-1.5 inline-flex align-baseline"
                    >
                      <VisibilityGlyph visibility={entry.visibility} />
                    </span>
                    {VISIBILITY_LABEL[entry.visibility]}
                  </p>
                </div>
                <div className="mt-3 grid gap-x-10 gap-y-4 md:grid-cols-[minmax(0,1.6fr)_minmax(0,2fr)]">
                  <div>
                    {/* The claim-line hierarchy (evviz): the ledger's own
                        short label — the number a reader scans for — set
                        in the display hand; the full claim stays serif
                        beneath it. Same strings, new order of arrival. */}
                    <h2 className="font-display fraunces-display text-ink max-w-[24ch] text-[1.45rem] leading-[1.2] tracking-[-0.01em]">
                      {entry.label}
                    </h2>
                    <p className="text-ink mt-3 max-w-[46ch] font-serif text-[1.0625rem] leading-6">
                      {entry.claim}
                    </p>
                  ...
                  <dl className="label-mono space-y-1.5">
                    ...
                    {/* evviz: the visibility pair moved to the record head
                      (glyph + the same bracketed label + sr-only key) —
                      one seat per fact, no duplicate row here. */}
                    <div>
