...
export function About({ themeId = "dark-luxe" }: AboutProps) {
  ...
  return (
    <section
      id="about"
      className="relative py-24 md:py-32 px-4 md:px-8 bg-background min-h-screen"
      data-theme={themeId}
      style={{
        backgroundColor:
          themeId === "paper-ink"
    ...
    >
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-16 flex flex-col items-center">
          ...
          />
          <h2
            className={`text-5xl md:text-6xl font-light text-foreground mb-4 text-center ${
        ...
        <div className="mb-12">
          {personalInfo.bio.map((paragraph, idx) => (
            <p
              key={idx}
              className={`text-lg text-foreground-muted mb-6 leading-relaxed last:mb-0 ${
        ...
        {education.length > 0 && (
          <div
            className={`mb-12 border p-8 rounded-sm transition-all duration-300 ${itemClass}`}
            style={{
          ...
          >
            ...
            {education.map((edu, idx) => (
              <div key={idx} className="mb-6 last:mb-0">
                ...
                <p className="text-accent-secondary text-base mb-4 flex items-center gap-2">
                  <MapPin size={16} />
                  {edu.school}
                </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-8 mb-12">
          <div className={`text-center border-b pb-6 transition-all duration-300 ${itemClass}`}>
            <div
        ...
        {/* Awards */}
        {awards.length > 0 && (
          <div className="pt-8" style={{ borderTop: "1px solid var(--accent-secondary)" }}>
            <h3
              className="text-2xl font-light text-accent-primary mb-6 flex items-center gap-3 uppercase tracking-wide"
              style={{
                fontFamily: "var(--font-display)",
                fontVariant: "small-caps",
              }}
            ...
            <div className="space-y-4">
              {awards.map((award, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-4 pb-4 border-b border-accent-secondary/10 last:border-b-0 transition-all duration-300 hover:translate-x-1"
                >
                  <div
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-foreground-muted text-sm mt-1">{award.description}</p>
                  </div>
                </div>
              ))}
            </div>
