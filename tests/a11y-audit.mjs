...
const DIR = '/sessions/intelligent-vigilant-lamport/mnt/Portfolio/portfolio/test-screenshots/a11y';
...
(async () => {
  ...
  for (const theme of themes) {
    ...
    const audit = await page.evaluate(() => {
      ...
      document.querySelectorAll('img').forEach(img => {
        if (!img.alt && !img.getAttribute('aria-hidden')) {
          issues.push({ type: 'missing-alt', element: img.src?.split('/').pop() || 'unknown', severity: 'critical' });
        }
      });
      ...
      document.querySelectorAll('a').forEach(a => {
        ...
        if (!name && !a.querySelector('img, svg')) {
          issues.push({ type: 'empty-link', href: a.href, severity: 'critical' });
        }
      });
      ...
      document.querySelectorAll('input, textarea, select').forEach(input => {
        ...
        if (!label && !ariaLabel && input.type !== 'hidden') {
          issues.push({ type: 'missing-label', inputType: input.type, severity: 'high' });
        }
      });
      ...
      document.querySelectorAll('h1, h2, h3, h4, h5, h6').forEach(h => {
        headings.push({ level: parseInt(h.tagName[1]), text: h.textContent?.slice(0, 50) });
      });
      let prevLevel = 0;
      headings.forEach(h => {
        if (h.level > prevLevel + 1) {
          issues.push({ type: 'heading-skip', from: `h${prevLevel}`, to: `h${h.level}`, text: h.text, severity: 'medium' });
        }
        prevLevel = h.level;
      });
      ...
      document.querySelectorAll('p, span, a, h1, h2, h3, h4, h5, h6, button, li').forEach(el => {
        const style = window.getComputedStyle(el);
        const color = style.color;
        const bg = style.backgroundColor;
        // Flag very light text on light bg or very dark text on dark bg
        if (color === bg && el.textContent?.trim()) {
          contrastIssues.push({ text: el.textContent?.slice(0, 30), color, bg });
        }
      });
      if (contrastIssues.length > 0) {
        issues.push({ type: 'potential-contrast', count: contrastIssues.length, severity: 'high' });
      }
    ...
    console.log(`  Headings: ${audit.headings.length}, Focusable: ${audit.focusableElements}`);
    audit.issues.forEach(i => console.log(`    [${i.severity}] ${i.type}: ${JSON.stringify(i).slice(0, 100)}`));
  }

  fs.writeFileSync(`${DIR}/audit-results.json`, JSON.stringify(results, null, 2));
  console.log('\nAudit complete!');
